#' @title The main function for probabilistic two-sample MR model for multiple outcome traits with summary-level data
#' @description moPMR_summary applies a likelihood-based approach, accounting for the correlated instruments and horizontal pleiotropy effect 
...
moPMR_summary<-function(Zscore_1, Zscore_2, Sigma1sin, Sigma2sin, Sigmayin, samplen1, samplen2, max_iterin =1000,epsin=1e-5, Heritability_geneexpression_threshold=1e-04){
  betaxin<-Zscore_1/sqrt(samplen1-1)
  betayin<-t(Zscore_2/sqrt(samplen2-1))
  fmH1=moPMR_summary_Egger(betaxin,betayin,Sigma1sin,Sigma2sin,Sigmayin,samplen1,samplen2,gammain=0,alphain=0,max_iterin =max_iterin, epsin=epsin)
  p=length(betaxin)
  k=dim(betayin)[1]
  ...
  if(Heritability_estimate <= Heritability_geneexpression_threshold){
    ...
    fmH0gamma=moPMR_summary_Egger(betaxin, betayin,Sigma1sin, Sigma2sin,Sigmayin, samplen1, samplen2, gammain=1,alphain=0, max_iterin =max_iterin, epsin=epsin)
    ...
    fmH0alpha=moPMR_summary_Egger(betaxin, betayin, Sigma1sin, Sigma2sin, Sigmayin,samplen1, samplen2, gammain=0, alphain=1,max_iterin =max_iterin, epsin=epsin)
    fmH0gamma=moPMR_summary_Egger(betaxin, betayin,Sigma1sin, Sigma2sin, Sigmayin,samplen1, samplen2, gammain=1,alphain=0, max_iterin =max_iterin, epsin=epsin)
    loglikH1=max(fmH1$loglik,na.rm=T)
