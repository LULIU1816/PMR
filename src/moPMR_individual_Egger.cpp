...
void lmm_pxem_ptr2(const arma::vec& x, const arma::mat& W, const arma::mat& Zx,  const int& maxIter,
...
void loglike_twas(const arma::mat& R, const arma::vec& res_x, double& res_ysum, const arma::vec& mu, const double& sigma2z, const double& sigma2x,
...
SEXP moPMR_individual_Egger(SEXP xin, SEXP Yin, SEXP Zxin, SEXP Zyin, SEXP gammain, SEXP alphain, SEXP max_iterin, SEXP epsin){// *
  try{
    ...
    lmm_pxem_ptr2(x, w1, Zx, maxIter , sigma2x , sigma2z , mux, loglik0 , iter0 , Sigb , mub);
    ...
    for (int iter = 2; iter <= maxIter; iter ++ ) {
     ...
     //cout << eVal << endl;
      ...
      // int l;
      // vec muty = zeros<vec>(p);
      // for(l=0;l<n2;l++)
      // muty = muty + (Zy.row(l)).t()*as_scalar(alpha.t()*sigmay_inv*(Y.col(l) - gamma*(w2.col(l))));
      // cout << muty << endl;
      ...
      loglike_twas(R, res_x, res_ysum, mu, sigma2z, sigma2x, eVal, n1 , n2, p, loglik(iter - 1));
